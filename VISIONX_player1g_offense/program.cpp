...
//				opponent_max_speed);
	...
	//the program below before the while loop is for finding the obstacles
	while(1) {
		...
		if (init<4) {
			...
			init ++;//so that any error about the position of robots to recheck
		}//-N->end of if
		...
		//-N->this below is for obstacle avoidance
...
int centroid_blue_op(image &rgb, int &icb, int &jcb){
	...
	for (j = 0; j < height; j++){
		for (i = 0; i < width; i++){
			...
			G = *(pb + 1);
			R = *(pb + 2);
			if (R <60 && G > 150 && B > 200){//must check the picture with irfan view to get the perfect 3-color intensity
				mb += B;
				mib += B*i;
				mjb += B*j;
			}
...
int point_robot(int icg, int jcg, int ico, int jco, double beta, int &PW_L, int &PW_R, double &a, int &l, int &PW_Laser){
	...
	//iro = (icb + ico) / 2;//thought so it targets the center of the opponent robot
	//jro = (jcb + jco) / 2;
...
int cover_object(image &grey, int is, int js)
{
	...
	// covering pixels in an -N->outward concentic ring pattern
